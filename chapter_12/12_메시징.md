...
- 그런 다음 다시 `Kernel_yield()` 함		수를 호출해서 Task1이 메시지 큐를 비우길 디다리고 같은 작업을 다시 시도한다. 
- `Kernel_send_msg()`가 두번 모두 실행되어야 while문을 빠져나간다. 
- 아래부분 부터 Task1의 핸들러 코드를 작성하도록 한다.
- Task0에서 보낸 메시지와 이벤트를 받는 부분이다.
- 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMjU0MDgzMjQsOTQ1MjE2ODc2LDE3NT
M3NDkzOTQsMTYwMDg0ODAxOSwtNDk3ODEyMDEwLDE0MTgxOTU0
NzgsMjEwOTUwODM1MiwtNzM0MDU4NDE0LDE1NTQxMzIyMDYsLT
MxODkyMDcwMSwtMTYzODgzMDI0NywtMTkzOTIxNjkxNywxMjc4
NzIwNDg4LC01NTc1ODQ5NTBdfQ==
-->